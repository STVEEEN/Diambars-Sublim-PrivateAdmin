...
const DesignStatsDotsContainer = styled(Box)(({ theme }) => ({
  display: 'flex',
  justifyContent: 'center',
  alignItems: 'center',
  gap: '8px',
...
const DesignStatsDot = styled(Box)(({ active, theme }) => ({
...
const DesignManagement = () => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const windowWidth = window.innerWidth;
      const isMobile = windowWidth <= 550;
      // Cuando hay 2 cards visibles, maxScroll es totalCards - 2 (2 posiciones para 4 cards: 0, 1)
  ...
  const handleStatsScroll = (e) => {
    ...
    if (isMobile) {
      // Una card por vez (100% del ancho) - 4 dots (0-3) para 4 cards
      cardWidth = containerWidth;
      maxIndex = totalCards - 1; // 3 (para 4 cards)
    } else {
      // Dos cards por vez (50% del ancho cada una + gap) - 3 dots (0-2) para 4 cards
    ...
    <DesignPageContainer>
      <DesignContentWrapper>
        ...
        <DesignStatsContainer>
          <DesignStatsGrid ref={statsGridRef} onScroll={handleStatsScroll}>
            {stats.map((stat) => (
              <DesignStatCard 
                key={stat.id} 
          ...
          <DesignSectionHeader>
            <DesignSectionTitle component="div">
              <Box sx={{ 
                ...
                <Chip 
                  label={`${designs.length}${pagination.total !== designs.length ? ` de ${pagination.total}` : ''}`}
                  size="small"
                  sx={{
                    background: alpha('#1F64BF', 0.1),
                    color: '#032CA6',
